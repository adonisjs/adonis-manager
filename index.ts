 ...
 *
 * This module is used all the place in AdonisJs. For example: `Mail`, `Sessions`,
 * `Auth` and so on.
 */
export abstract class Manager<T> {
   ...
   */
  protected _driversCache = {}

  /**
   * List of drivers added at runtime
   */
  protected _extendedDrivers: { [key: string]: (app) => T } = {}

  /**
   * Whether or not to cache drivers
   */
  protected abstract _cacheDrivers: boolean
   ...
   * `_cacheDrivers` attribute before entertaining the cache
   */
  private _getCachedDriver (name): T | null {
    if (this._cacheDrivers && this._driversCache[name]) {
